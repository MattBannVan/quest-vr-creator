...
**Status as of Grok automation session (2026-07-31):** Full bidirectional material panel + object scale live controls + particle/glow feedback on spawn/delete implemented and validated. 5-row taller tablet with M± R± O± S± buttons, reactive O + Sc stats, keyboard [ ] scale, spawn respects selectedScale, emissive pulse feedback. All 4 hooks + index syntax-validated (node --check zero errors), consistent design preserved. **Docs accuracy enforced as first-class rule.**
...
- [x] Desktop keyboard fallbacks (1-5 spawn, U undo, M mat, S save, L load, E JSON, G GLTF, X share, H holo, D del, Ctrl+C clear, o/Shift+o for opacity ±0.1, [ ] / +/- for scale)
...
- [x] **Full bidirectional material panel on tablet** (M- R- O+ O- buttons + taller 5-row layout + reactive O in stats) — handlers + state already existed; now fully UI-complete and consistent
...
5. [ ] Object rotation live controls or multi-select
